      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {[{ label: 'All', value: '' }, ...Object.entries(STATUS_CONFIG).map(([k, v]) => ({ label: v.label, value: k }))].map(({ label, value }) => (
          <button
            ...
            className="px-4 py-2 rounded-2xl text-sm font-bold whitespace-nowrap transition-all duration-300 flex-shrink-0"
            style={{
              background: filterStatus === value ? '#0f172a' : '#ffffff',
              color: filterStatus === value ? '#ffffff' : '#64748b',
              border: `1px solid ${filterStatus === value ? '#0f172a' : '#e2e8f0'}`,
              boxShadow: filterStatus === value ? '0 4px 12px rgba(15,23,42,0.2)' : '0 1px 3px rgba(0,0,0,0.05)',
            }}
          >
            {label}
          </button>
